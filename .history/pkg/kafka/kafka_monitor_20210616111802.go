...
func main() {
	...
	config, err := clientcmd.BuildConfigFromFlags("", "/data/zejia.lu/.kube/config-x5")
	if err != nil {
		panic(err)
	}
	...
	pods, _ := clientset.CoreV1().Pods("prj-luzejia-test").List(context.Background(), metav1.ListOptions{})
	...
	pvcs, _ := clientset.CoreV1().PersistentVolumeClaims("prj-luzejia-test").List(context.Background(), metav1.ListOptions{})
	...
	podMetrics, err := mc.MetricsV1beta1().PodMetricses("prj-luzejia-test").List(context.Background(), metav1.ListOptions{})
	if err != nil {
