...
func Count() {
	...
	fmt.Println("\nkun every namespace message status:")
	for cluster, cluster_namespace := range namespacecount {
		...
		for namesapce, value := range cluster_namespace {
			...
			fmt.Printf("namespace: %-50s, message_count: %-12d, message_size: %10dMb\n", namesapce, value["message_num"], value["message_size"]/1024)
		}
	}

	fmt.Println("\nkun every cluster message status:")
	for key, value := range cluster_count {
		...
		fmt.Printf("cluster: %-8s, message_count: %-12d, message_size: %10dGb\n", key, value["message_num"], value["message_size"]/1024/1024)
	}
	...
	fmt.Printf("\n%s kun all message message count:%-11d, message size:%10fTb\n", yesterday, NumCount, FloatRound(float64(SizeCount)/1024/1024/1024, 2))
}
