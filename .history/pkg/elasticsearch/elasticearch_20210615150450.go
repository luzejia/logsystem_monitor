...
func Count() {
	...
	for _, index := range result {
		if strings.Contains(index.Name, yesterday) {
			...
			if cluster_count[index.Name[:7]] == nil {
				cluster_count[index.Name[:7]] = make(map[string]int64)
			}
			cluster_count[index.Name[:7]]["message_num"] += messagenum
			cluster_count[index.Name[:7]]["message_size"] += indexSize
		}
	}

	fmt.Println("kun every namespace message status:")
	for key, value := range namespacecount {
		fmt.Println("namespace:", key, ",", " message_count:", value["message_num"], ",", " message_size:", value["message_size"], "Mb")
	}

	fmt.Println("kun every cluster message status:")
	for key, value := range cluster_count {
		fmt.Println("cluster:", key, ",", " message_count:", value["message_num"], ",", " message_size:", value["message_size"]/1024, "Gb")
	}

	fmt.Println(yesterday, "kun all namesapce message size:", SizeCount/1024/1024, "Tb", "kun all message message count:", NumCount)
